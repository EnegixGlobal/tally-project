...
const Header: React.FC = () => {
  const { sidebarOpen, setSidebarOpen, logout, adminData } = useAdmin();
        ...
        <div className="flex items-center space-x-4">
          ...
          <div className="relative hidden sm:block">
            ...
            <input
              type="text"
              placeholder="Search..."
              className={`pl-10 pr-4 py-2 border rounded-lg focus:ring-2 focus:ring-primary focus:border-primary w-64 lg:w-80 ${theme === 'dark'
        ...
        <div className="flex items-center space-x-4">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div className="text-sm hidden md:block">
              <div className={`font-medium ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>{adminData?.email}</div>
              <div className={`${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}>{adminData?.role}</div>
            </div>
          </div>
          ...
          <button
            ...
            className={`p-2 rounded-lg ${theme === 'dark' ? 'hover:bg-gray-700 text-red-500' : 'hover:bg-red-50 text-red-600'} transition-colors ml-2`}
          >
