...
const ForgotPassword: React.FC = () => {
  ...
  const handleSendEmail = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      // Backend /api/auth/forgot (or wherever the main auth route is accessible for admin). 
      // Wait, is it /api/admin/forgot? Let's check where the backend is mounted.
      // Wait, the regular user forgotPassword routes are in /auth/forgot.
      const response = await api.post('/api/auth/forgot', { email: email.trim() });
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const response = await api.post('/api/auth/verify-otp', { otpToken, otp: otp.trim() });
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center bg-gradient-to-br from-blue-500/80 via-purple-600/80 to-purple-800/80 bg-[url('/src/assets/bg-1.svg')] bg-cover bg-center bg-no-repeat bg-blend-overlay p-4 xl:p-8">
