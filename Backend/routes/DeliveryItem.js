...
router.post('/', async (req, res) => {
    const { date, number, narration, companyId, ownerType, ownerId, entries } = req.body;

    const [deliveryResult] = await db.query(
        `INSERT INTO delivery_items (date, number, narration, company_id, owner_type, owner_id) VALUES (?, ?, ?, ?, ?, ?)`,
