...
router.post("/transfer", async (req, res) => {
  const { transactionId, voucherType, oldLedgerId, newLedgerId } = req.body;

  if (!transactionId || !voucherType || !oldLedgerId || !newLedgerId) {
  ...
  try {
    ...
    const vTypeLower = String(voucherType).toLowerCase();

    if (["receipt", "payment", "journal", "contra"].includes(vTypeLower)) {
      const rawId = transactionId.includes("-") ? transactionId.split("-").pop() : transactionId;
      await connection.execute(
        `UPDATE voucher_entries SET ledger_id = ? WHERE voucher_id = ? AND ledger_id = ?`,
        [newLedgerId, rawId, oldLedgerId]
      );
    } else if (vTypeLower === "sales") {
      const rawId = transactionId.includes("-") ? transactionId.split("-").pop() : transactionId;
      await connection.execute(
        `UPDATE sales_vouchers SET partyId = CASE WHEN partyId = ? THEN ? ELSE partyId END WHERE id = ?`,
        [oldLedgerId, newLedgerId, rawId]
      );
      await connection.execute(
        `UPDATE sales_voucher_items SET salesLedgerId = CASE WHEN salesLedgerId = ? THEN ? ELSE salesLedgerId END, discountLedgerId = CASE WHEN discountLedgerId = ? THEN ? ELSE discountLedgerId END WHERE voucherId = ?`,
        [oldLedgerId, newLedgerId, oldLedgerId, newLedgerId, rawId]
      );
      await connection.execute(
        `UPDATE voucher_entries SET ledger_id = ? WHERE voucher_id = ? AND ledger_id = ?`,
        [newLedgerId, rawId, oldLedgerId]
      );
    } else if (vTypeLower === "purchase") {
      const rawId = transactionId.includes("-") ? transactionId.split("-").pop() : transactionId;
      await connection.execute(
        `UPDATE purchase_vouchers SET partyId = CASE WHEN partyId = ? THEN ? ELSE partyId END WHERE id = ?`,
        [oldLedgerId, newLedgerId, rawId]
      );
      await connection.execute(
        `UPDATE purchase_voucher_items SET purchaseLedgerId = CASE WHEN purchaseLedgerId = ? THEN ? ELSE purchaseLedgerId END, discountLedgerId = CASE WHEN discountLedgerId = ? THEN ? ELSE discountLedgerId END WHERE voucherId = ?`,
        [oldLedgerId, newLedgerId, oldLedgerId, newLedgerId, rawId]
      );
      await connection.execute(
        `UPDATE voucher_entries SET ledger_id = ? WHERE voucher_id = ? AND ledger_id = ?`,
