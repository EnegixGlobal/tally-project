  ...
  try {
    ...
    const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${apiKey}`, {
      ...
      body: JSON.stringify({
        contents: [{
          parts: [
            { text: "Extract detailed bill data. Return ONLY a JSON object exactly structured as follows: { \"invoiceNumber\": string, \"date\": string (YYYY-MM-DD), \"supplierName\": string, \"supplierGst\": string, \"supplierAddress\": string, \"supplierState\": string, \"supplierPinCode\": string, \"supplierPan\": string, \"totalTaxableValue\": number, \"totalIgst\": number, \"totalCgst\": number, \"totalSgst\": number, \"tdsAmount\": number, \"discountAmount\": number, \"items\": [{ \"name\": string, \"hsnSac\": string, \"quantity\": number, \"unit\": string, \"rate\": number, \"taxableValue\": number, \"igstAmount\": number, \"cgstAmount\": number, \"sgstAmount\": number, \"discount\": number, \"purchaseLedgerHint\": string }] }. If any field is not found, use null or 0. Do not include markdown formatting like ```json." },
