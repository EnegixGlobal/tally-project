...
router.get('/api/cash-flow', async (req, res) => {
  const { financialYear, startDate, endDate, company_id, owner_type, owner_id } = req.query;
  ...
  try {
    ...
    WHERE
      vm.company_id = ? AND vm.owner_type = ? AND vm.owner_id = ?
...
const [inflowRows] = await pool.query(inflowQuery, [...inflowTypes, company_id, owner_type, owner_id, start, end]);
const [outflowRows] = await pool.query(outflowQuery, [...outflowTypes, company_id, owner_type, owner_id, start, end]);
