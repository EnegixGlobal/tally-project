...
router.post("/", async (req, res) => {
  const { 
    employee_id, company_id, assessee, salary, business, houseProperty,
  ...
  try {
    ...
    const [result] = await connection.query(
      ...
      life_insurance_premium, tuition_fee_1st2nd_child, tds_deducted, company_id)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?,
              ?, ?, ?,
              ?, ?, ?, ?, ?,
              ?, ?, ?, ?, ?, ?, ?, ?,
...
router.get("/:employee_id", async (req, res) => {
  ...
  try {
    const [statements] = await db.query(
      "SELECT * FROM itr_statements WHERE employee_id = ? AND company_id = ?",
