...
router.post('/', async (req, res) => {
  const { date, number, mode, partyId, narration, entries, companyId, ownerType, ownerId } = req.body;

  console.log('--- /api/CreditNotevoucher START ---');
  console.log('Body:', req.body);

  const mainVoucherQuery = `
    INSERT INTO credit_vouchers (\`date\`, \`number\`, \`mode\`, \`partyId\`, \`narration\`, \`company_id\`, \`owner_type\`, \`owner_id\`)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)`;
  ...
  try {
    ...
    const [mainResult] = await connection.query(mainVoucherQuery, [
     date, number, mode, partyId, narration, companyId, ownerType, ownerId
    ]);
