...
const JournalRegister: React.FC = () => {
  ...
  const handleGenerateXml = async (voucher: any) => {
    try {
      ...
      const xmlContent = generateJournalXmlContent(fullVoucher, companyName, ledgers);
  ...
  const handleGenerateAllXml = async (vouchers: any[]) => {
    ...
    try {
      ...
      const xmlContent = generateBulkJournalXmlContent(fullVouchers, companyName, ledgers);
