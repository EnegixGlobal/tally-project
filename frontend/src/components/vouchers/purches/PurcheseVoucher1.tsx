...
const PurchaseVoucher: React.FC = () => {
  ...
  const partyLedgers = ledgers.filter(l => l.groupName === 'Sundry Creditors' || l.groupName === 'Cash' || l.groupName === 'Current Assets');
  const purchaseLedgers = ledgers.filter(l => l.groupName === 'Purchase' || l.groupName === 'Direct Expenses' || l.groupName === 'Expense');
...
const selectedItem = stockItems.find(item => String(item.id) === String(value));
      ...
      updatedEntries[index] = {
        ...
        sgstRate: gstRate / 2,
        igstRate: 0,
        // rate: selectedItem?.rate ?? 0, // set default rate if desired
        hsnCode: selectedItem?.hsnCode || '', // optional
        // unit: selectedItem?.unit || '', // optional
      };
                ...
                <table className="w-full mb-4">
                  ...
                  <tbody>                    
                    ...
                    <tr className={`font-semibold ${theme === 'dark' ? 'border-t border-gray-600' : 'border-t border-gray-300'}`}>
                      ...
                      <td className="px-4 py-2 text-right">{cgstTotal.toLocaleString()} + {sgstTotal.toLocaleString()}
</td>
                      <td className="px-4 py-2"></td>
                      <td className="px-4 py-2"></td>
                    </tr>
