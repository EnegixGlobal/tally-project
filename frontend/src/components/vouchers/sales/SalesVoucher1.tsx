...
import type { VoucherEntry, Ledger, Godown, LedgerWithGroup } from '../../../types';
...
const SalesVoucher: React.FC = () => {
  const { theme, godowns = [], vouchers = [], companyInfo, addVoucher, updateVoucher } = useAppContext();
...
const partyLedgers = ledgers.filter(l => l.groupName === 'Sundry Debtors' || l.groupName === 'Cash');
...
const handleSubmit = async (e: React.FormEvent) => {
  ...
  try {
    ...
    if (isEditMode && id) {
      ...
      const res = await fetch(`http://localhost:5000/api/sale-vouchers/${id}`, {
        method: 'PUT',
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              <div>
                ...
                >
                  ...
                  {ledgers.filter(l => l.type === 'sales' || l.name.toLowerCase().includes('sales')).map(ledger => (
                    <option key={ledger.id} value={ledger.id}>{ledger.name}</option>
                  ))}
                <option value="add-new" className={`flex items-center px-4 py-2 rounded ${
                              theme === 'dark' ? 'bg-blue-600 hover:bg-green-700' : 'bg-green-600 hover:bg-green-700 text-white'
