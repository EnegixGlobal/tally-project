...
const PurchaseImport: React.FC = () => {
    ...
    const processFile = async (file: File) => {
        ...
        try {
            ...
            const processedRows: ImportedRow[] = jsonData.map((row) => {
                ...
                const isFullyMatched = !!(matchedLedgerByName && gstMatch && stateMatch);
    ...
    const saveImportedVouchers = async () => {
        ...
        try {
            ...
            for (let i = 0; i < updatedRows.length; i++) {
                ...
                try {
                    const response = await axios.post(`${import.meta.env.VITE_API_URL}/api/vouchers/purchase_summary_import`, {
                        rows: [updatedRows[i]],
                        companyId,
                        ownerType,
                        ownerId
            ...
            if (savedCount > 0) {
                Swal.fire({
                    ...
                    text: `${savedCount} Purchase Vouchers imported successfully.`,
                    timer: 3000
                });
            }
        } catch (err) {
    ...
    const downloadTemplate = () => {
        ...
        const dummyData = [
            ["20AABCM4621M1ZR", "MONGIA STEEL LIMITED", "MSL/25-26/14420", "16-02-2026", 938100, "Jharkhand", "18% Inter State", 18, 795000, 0, 71550, 71550]
        ];
                                            ...
                                            <tr key={i} className="hover:bg-gray-50 transition-colors">
                                                ...
                                                <td className="px-6 py-4">
                                                    <div className="flex flex-col">
                                                        <span className={`font-semibold ${!row.partyMatch ? "text-red-600" : "text-gray-900"}`}>
