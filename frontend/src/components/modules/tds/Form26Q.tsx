...
React.useEffect(() => {
  const fetchReturns = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/tds26q?year=${selectedYear}`);
  ...
  try {
    const res = await fetch(`${import.meta.env.VITE_API_URL}/api/tds26q`, {
      method: "POST",
...
function setReturnsList(_data: any) {
