...
import { Settings, Users, Shield, Monitor, User } from "lucide-react"; //Database,Printer, Mail, RefreshCcw
...
const ConfigModule: React.FC = () => {
      ...
      items: [
        {
          icon: <Users size={20} />,
          name: "User Accounts",
          path: "/app/config/users",
        },
        {
          icon: <Shield size={20} />,
          name: "Permissions",
          path: "/app/config/permissions",
        },
        {
          icon: <Settings size={20} />,
  ...
  return (
    <div className="pt-[56px] px-4 ">
                ...
                >
                  <div
                    className={`p-2 rounded-full mb-2 ${
                      theme === "dark" ? "bg-gray-600" : "bg-indigo-50"
                    }`}
                  >
                    {item.icon}
                  </div>
      ...
      >
        <p className="text-sm">
          <span className="font-semibold">Pro Tip:</span> Press Alt+F12 to
          quickly access Configuration settings.
        </p>
      </div>
    </div>
  );
