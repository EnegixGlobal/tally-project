...
const SalesInvoiceMatching: React.FC = () => {
  ...
  const { mergedData: mergedRows, matchStats } = React.useMemo(() => {
    // 1. Base Merge
    const baseRows = salesVouchers.map(voucher => {
      const historyItems = salesHistory.filter(h =>
    ...
    const partyKey = findKey([/party\s*name/i, /buyer\s*name/i, /customer\s*name/i, /ledger\s*name/i]);
    ...
    const matchedRows = baseRows.map(row => {
      ...
      if (excelRow) {
        ...
        if (dateKey && status === 'MATCHED') {
          ...
          // User said "jo jo data ui me show kar raha hai... strict match".
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-[60vh] gap-6">
            ...
            <div className="text-center space-y-2">
              ...
              <p className="text-sm opacity-70">Comparing {importedData.length} excel rows with {salesVouchers.length} book vouchers.</p>
            </div>
          </div>
        ) : (
