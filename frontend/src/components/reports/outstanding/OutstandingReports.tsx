    ...
    async function fetchSummary() {
      ...
      try {
        const res = await fetch(
          `${import.meta.env.VITE_API_URL}/api/outstanding-summary`
        );
    ...
    <div className="pt-[56px] px-4">
          ...
          {sections.map((section, index) => {
            ...
            return (
              <button
                key={section.id}
                onClick={() => {
                  if (!isDisabled) handleSectionClick(section.id);
                }}
                disabled={isDisabled}
                className={`
      ...
      `}
              >
                {/* Coming Soon badge */}
                {isDisabled && (
                  <span className="absolute top-3 right-3 text-xs px-2 py-1 rounded-full bg-black/70 text-white">
                    Coming Soon
                  </span>
                )}
        ...
        >
          ...
          {summaryError && (
            <div className="text-center text-red-600 mt-2">{summaryError}</div>
          )}
        </div>
      </div>
    </div>
