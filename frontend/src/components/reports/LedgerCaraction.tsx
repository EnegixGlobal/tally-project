...
const LedgerCaraction: React.FC = () => {
  ...
  // Find all Bank Account groups recursively
  const bankAccountGroupIds = useMemo(() => {
    const bankGroups = ledgerGroups.filter(g => 
      g.name.toLowerCase().includes("bank")
    );
    
    const ids = new Set<number>(bankGroups.map(g => g.id));
    let added = true;
    while (added) {
      added = false;
  ...
  const bankLedgers = useMemo(() => {
    return ledgers.filter(l => bankAccountGroupIds.has(Number(l.groupId)));
  }, [ledgers, bankAccountGroupIds]);
  ...
  useEffect(() => {
    ...
    fetch(
      `${import.meta.env.VITE_API_URL
      }/api/ledger-caraction/report?ledgerId=${ledgerId}&fromDate=${fromDate}&toDate=${toDate}&includeOpening=${includeOpening}&includeClosing=${includeClosing}`
    )
      .then((res) => res.json())
  ...
  }, [ledgerId, fromDate, toDate, includeOpening, includeClosing, reloadTrigger]);
  ...
  const handleTransfer = async (txn: LedgerTransaction, newLedgerId: string) => {
    if (!newLedgerId || newLedgerId === ledgerId) return;

    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/ledger-caraction/transfer`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          transactionId: txn.id,
          voucherType: txn.voucherType,
          oldLedgerId: ledgerId,
      ...
      console.error(err);
      alert("Error transferring entry");
    }
  };
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  // URL me bhi bhejo
                  navigate(`/app/reports/ledger-caraction/${id}`);
                }}
                ...
                <table className="w-full border-collapse">
                  ...
                  <tbody>
                              ...
                              <td className="px-4 py-3 text-sm text-right font-mono font-medium">
                                ...
                                <td className="px-4 py-3 flex items-center justify-center space-x-2">
                                  ...
                                  <button
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      if (transferSelections[first.id]) {
                                        handleTransfer(first, transferSelections[first.id]);
                                      } else {
                    ...
                    {groupedByVoucher.length === 0 && (
                      <tr>
                        <td colSpan={9} className="px-4 py-8 text-center border-b border-gray-200 dark:border-gray-700">
...
export default LedgerCaraction;
