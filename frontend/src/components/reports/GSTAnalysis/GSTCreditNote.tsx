...
const GSTCreditNote: React.FC = () => {
    ...
    // ================================
    // Fetch Credit Note Ledgers
    // ================================
    useEffect(() => {

        if (!company_id || !owner_type || !owner_id) return;

        const fetchCreditNoteLedgers = async () => {
            try {

                const res = await fetch(
                    `${import.meta.env.VITE_API_URL}/api/gst-assessment/credit-note?company_id=${company_id}&owner_type=${owner_type}&owner_id=${owner_id}`
                );
                ...
                console.error("Credit Note Ledger Fetch Error:", err);
            }
        };

        fetchCreditNoteLedgers();
                ...
                <table className="w-full border-collapse text-center text-sm">
                    ...
                    <thead className="bg-gray-800 text-white">
                        ...
                        <tr>
                            ...
                            {/* Inter State Credit Note Group */}
                            <th
                                colSpan={(interSalesLedgers.length || 1) + 1}
                                className="border border-gray-600 px-3 py-2"
                            >
                                Credit Note (Inter State)
                            </th>
