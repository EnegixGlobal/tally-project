...
const GSTDebitNote: React.FC = () => {
  ...
  // ================================
  // Fetch Debit Note Ledgers
  // ================================
  useEffect(() => {

    if (!company_id || !owner_type || !owner_id) return;

    const fetchDebitNoteLedgers = async () => {
      try {

        const res = await fetch(
          `${import.meta.env.VITE_API_URL}/api/gst-assessment/debit-note?company_id=${company_id}&owner_type=${owner_type}&owner_id=${owner_id}`
        );
        ...
        console.error("Debit Note Ledger Fetch Error:", err);
      }
    };

    fetchDebitNoteLedgers();
        ...
        <table className="w-full border-collapse text-center text-sm">
          ...
          <thead className="bg-gray-800 text-white">
            ...
            <tr>
              ...
              {/* Intra State Debit Note Group */}
              <th
                colSpan={(intraPurchaseLedgers.length || 1) + 1}
                className="border border-gray-600 px-3 py-2"
              >
                Debit Note (Intra State)
              </th>

              {/* Inter State Debit Note Group */}
              <th
                colSpan={(interPurchaseLedgers.length || 1) + 1}
                className="border border-gray-600 px-3 py-2"
              >
                Debit Note (Inter State)
              </th>
              ...
              return (
                <tr key={month} className="odd:bg-white even:bg-gray-50 hover:bg-gray-200 transition-colors">
                  ...
                  <td className="border border-gray-300 px-3 py-2 font-semibold bg-gray-100">
                    {mData.totalIntraPurchase ? Number(mData.totalIntraPurchase).toFixed(2) : ""}
                  </td>
                  ...
                  {/* Inter Purchase Total */}
                  <td className="border border-gray-300 px-3 py-2 font-semibold bg-gray-100">
                    {mData.totalInterPurchase ? Number(mData.totalInterPurchase).toFixed(2) : ""}
                  </td>
          ...
          <tfoot className="bg-gray-800 text-white font-bold">
            <tr>
              ...
              <td className="border border-gray-600 px-3 py-2">
                {(() => {
                  const total = months.reduce((acc, month) => acc + (Number(monthlyData[month]?.totalIntraPurchase) || 0), 0);
              ...
              <td className="border border-gray-600 px-3 py-2">
                {(() => {
                  const total = months.reduce((acc, month) => acc + (Number(monthlyData[month]?.totalInterPurchase) || 0), 0);
                  return total ? total.toFixed(2) : "";
                })()}
              </td>
