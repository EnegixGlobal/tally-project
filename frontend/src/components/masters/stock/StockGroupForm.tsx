...
const StockGroupForm: React.FC = () => {
  const { theme, stockGroups, gstClassifications, companyInfo } =
    useAppContext();
  ...
  const validateForm = useCallback(() => {
    ...
    if (
      ...
      !formData.hsnSacDetails.hsnCode
    ) {
      newErrors.hsnCode = "HSN/SAC code is required when set/alter is enabled";
    }
    if (
      formData.gstDetails?.setAlterGST &&
      !formData.gstDetails.gstClassificationId &&
      !formData.gstDetails.integratedTaxRate
    ) {
        ...
        .then((data) => {
          if (data) {
            setFormData({
              ...
              ...data,
              hsnSacDetails:
                data.hsnSacDetails || initialFormData.hsnSacDetails,
              gstDetails: data.gstDetails || initialFormData.gstDetails,
            });
          }
        })
        .catch((err) => console.error("Failed to fetch stock group:", err));
    }
  }, [id, isEditMode]);
      ...
      printWindow.document.write(`
        <html>
          ...
          <body>
            ...
            } - Stock Group Details</h1>
            <table>
              <tr><th>Name</th><td>${formData.name}</td></tr>
              <tr><th>Parent Group</th><td>${
                stockGroups.find((g) => g.id === formData.parent)?.name ||
                "None"
              }</td></tr>
              <tr><th>Should Quantities be Added</th><td>${
                formData.shouldQuantitiesBeAdded ? "Yes" : "No"
              }</td></tr>
              <tr><th>Set/Alter HSN/SAC</th><td>${
                formData.hsnSacDetails?.setAlterHSNSAC ? "Yes" : "No"
              }</td></tr>
              ${
                formData.hsnSacDetails?.setAlterHSNSAC
                  ? `
                <tr><th>HSN/SAC Code</th><td>${
                  formData.hsnSacDetails.hsnCode
                }</td></tr>
                <tr><th>HSN/SAC Description</th><td>${
                  formData.hsnSacDetails.description || "N/A"
                }</td></tr>
              `
                  : ""
              }
              <tr><th>Set/Alter GST Details</th><td>${
                formData.gstDetails?.setAlterGST ? "Yes" : "No"
              }</td></tr>
              ${
                formData.gstDetails?.setAlterGST
                  ? `
                <tr><th>GST Classification</th><td>${
                  gstClassifications.find(
                    (c) => c.id === formData.gstDetails?.gstClassificationId
                  )?.name || "Manual"
                }</td></tr>
                <tr><th>Taxability</th><td>${
                  formData.gstDetails?.taxability
                }</td></tr>
                <tr><th>GST Rate</th><td>${
                  formData.gstDetails?.integratedTaxRate
                }%</td></tr>
                <tr><th>Cess</th><td>${
                  formData.gstDetails?.cess || 0
                }%</td></tr>
              `
                  : ""
              }
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            >
              Name Under (Parent Group/Category)
            </label>
            <select
              name="parent"
              title="Name Under (Parent Group/Category)"
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setFormData((prev) => ({
                  ...prev,
                  shouldQuantitiesBeAdded: e.target.value === "Yes",
                }))
              }
              className={`w-full p-2 rounded border ${
                theme === "dark"
